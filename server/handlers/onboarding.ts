...
export function postData(request: Request, response: Response) {
  ...
  pool.query(creating_user, [name, onboarding], async (err, result) => {
    if (err) {
      ...
      response.status(201).json(result.rows[0] as Number);
    }
  });
}

export function fetchData(request: Request, response: Response) {
...
export function editData(request: Request, response: Response) {
...
export function deleteData(request: Request, response: Response) {
  const id = request.params.id;
